...
describe("check that product, order and user functionality works as expected", () => {
  ...
  it("can create new users and display them", () => {
    const myUser = {
      name: Cypress.env("newUserName"),
      email: Cypress.env("newUserEmail"),
      role: Cypress.env("newUserRole"),
    };
  ...
  it(
    "can create a new order with a new product and see them listed",
    {
      retries: {
        runMode: 3,
        openMode: 2,
    ...
    () => {
      ...
      cy.get("table td").contains(
        new Intl.NumberFormat().format(myProduct.price * 2)
      );
